...
const HomePage = () => {
  ...
  const [boardFormSuccess, setBoardFormSuccess] = useState('');
  ...
  // Helper function to calculate timeline duration and determine background color
  const getTimelineColor = (start, end) => {
    if (!start || !end) return 'bg-gray-200'; // Default color if dates are missing
    const startDate = new Date(start);
    const endDate = new Date(end);
    const diffInMonths = (endDate - startDate) / (1000 * 60 * 60 * 24 * 30); // Approximate months
    const diffInYears = diffInMonths / 12; // Approximate years

    if (diffInMonths < 1) {
      return 'bg-red-500 text-white'; // Less than 1 month
    } else if (diffInYears < 1) {
      return 'bg-yellow-500 text-white'; // Less than 1 year
    } else {
      return 'bg-green-500 text-white'; // 1 year or more
    }
  };
  ...
  useEffect(() => {
    ...
    getBoards()
      ...
      .catch(() => setBoardFormError('Lỗi tải danh sách board'));
  }, []);
  ...
  useEffect(() => {
    if (selectedBoard) {
      ...
      getGroupsByBoard(selectedBoard.id)
        ...
        .catch(() => setGroupFormError('Lỗi tải danh sách group'));
    } else {
      setGroups([]);
  ...
  useEffect(() => {
    if (selectedGroup) {
      ...
      getTasksByGroup(selectedGroup.id)
        ...
        .catch(() => setTaskFormError('Lỗi tải danh sách task'));
    } else {
      setTasks([]);
      setLoadingTasks(false);
    }
  ...
  const handleAddBoard = async (e) => {
    ...
    try {
      ...
      setBoardFormSuccess('Thêm board thành công');
      setTimeout(() => setBoardFormSuccess(''), 3000);
    } catch (error) {
      setBoardFormError(error?.response?.data || 'Lỗi thêm board');
    }
  };
  ...
  const handleUpdateBoard = async (e) => {
    ...
    try {
      ...
      setBoardForm({ name: '' });
      setShowBoardModal(false);
      setBoardFormSuccess('Cập nhật board thành công');
      setTimeout(() => setBoardFormSuccess(''), 3000);
    } catch (error) {
      setBoardFormError(error?.response?.data || 'Lỗi cập nhật board');
    }
  };
  ...
  const handleDeleteBoard = async (id) => {
    try {
      ...
      setBoardFormSuccess('Xóa board thành công');
      setTimeout(() => setBoardFormSuccess(''), 3000);
    } catch (error) {
      setBoardFormError(error?.response?.data || 'Lỗi xóa board');
    }
  };
  ...
  const handleAddGroup = async (e) => {
    ...
    try {
      ...
      setGroupFormSuccess('Thêm group thành công');
      setTimeout(() => setGroupFormSuccess(''), 3000);
    } catch (error) {
      setGroupFormError(error?.response?.data || 'Lỗi thêm group');
    }
  };
  ...
  const handleUpdateGroup = async (e) => {
    ...
    try {
      ...
      setGroupForm({ name: '' });
      setShowGroupModal(false);
      setGroupFormSuccess('Cập nhật group thành công');
      setTimeout(() => setGroupFormSuccess(''), 3000);
    } catch (error) {
      setGroupFormError(error?.response?.data || 'Lỗi cập nhật group');
    }
  };
  ...
  const handleDeleteGroup = async (id) => {
    try {
      ...
      setGroupFormSuccess('Xóa group thành công');
      setTimeout(() => setGroupFormSuccess(''), 3000);
    } catch (error) {
      setGroupFormError(error?.response?.data || 'Lỗi xóa group');
    }
  };
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      ...
      setTaskFormSuccess('Thêm task thành công');
      setTimeout(() => setTaskFormSuccess(''), 3000);
    } catch (error) {
      setTaskFormError(error?.response?.data || 'Lỗi thêm task');
    }
  };
  ...
  const handleUpdateTask = async (e) => {
    ...
    try {
      ...
      setEditTask(null);
      setShowTaskModal(false);
      setTaskFormSuccess('Cập nhật task thành công');
      setTimeout(() => setTaskFormSuccess(''), 3000);
    } catch (error) {
      setTaskFormError(error?.response?.data || 'Lỗi cập nhật task');
    }
  };
  ...
  const handleDeleteTask = async (id) => {
    try {
      ...
      setTaskFormSuccess('Xóa task thành công');
      setTimeout(() => setTaskFormSuccess(''), 3000);
    } catch (error) {
      setTaskFormError(error?.response?.data || 'Lỗi xóa task');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 flex relative">
      ...
      >
        ...
        <div className="px-4 py-4 flex-1 overflow-y-auto">
          <div className="mb-6">
            {boardFormSuccess && (
              <div className="bg-green-100 border-l-4 border-green-500 text-green-700 p-3 mb-4 rounded-r-lg animate-pulse">
                {boardFormSuccess}
              </div>
            )}
            <select
                      ...
                      <tr key={t.id} className="border-b hover:bg-gray-50 transition duration-200">
                        ...
                        <td className="p-4">
                          {t.timelineStart && t.timelineEnd ? (
                            <span
                              className={`${getTimelineColor(t.timelineStart, t.timelineEnd)} px-3 py-1 rounded-full text-xs`}
                            >
