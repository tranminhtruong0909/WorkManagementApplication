...
public class BoardService {
    ...
    public void assignUserRole(User currentUser, Integer boardId, AssignRoleRequest req) {
        ...
        // Nếu user đang tự gán role cho chính mình (trường hợp tạo board mới)
        if (currentUser.getId().equals(req.getUserId())) {
            System.out.println("DEBUG: User assigning role to themselves, skipping permission check");
        } else {
            // Kiểm tra currentUser có phải ADMIN không (chỉ khi gán cho user khác)
            UserRole selfRole = userRoleRepo.findByUserIdAndBoardId(currentUser.getId(), boardId)
        ...
        Optional<UserRole> existing = userRoleRepo.findByUserIdAndBoardId(userToUpdate.getId(), boardId);

        if (existing.isPresent()) {
            UserRole role = existing.get();
            role.setRole(newRole);
            userRoleRepo.save(role);
            System.out.println("DEBUG: Updated existing role for user " + userToUpdate.getId() + " to " + newRole);
        } else {
            ...
            role.setRole(newRole);
            userRoleRepo.save(role);
            System.out.println("DEBUG: Created new role for user " + userToUpdate.getId() + " as " + newRole);
        }
    }
}
