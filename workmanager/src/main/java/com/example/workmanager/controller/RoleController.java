...
import com.example.workmanager.security.CustomUserDetails;
...
public class RoleController {
    ...
    public ResponseEntity<List<UserRole>> getAllUsersInBoard(@PathVariable Integer boardId,
                                                             @AuthenticationPrincipal CustomUserDetails userDetails) {
        User user = userDetails.getUser();
        if (!permissionService.canManageBoard(user.getId(), boardId)) {
            return ResponseEntity.status(403).body("Bạn không có quyền thực hiện chức năng này!");
    ...
    public ResponseEntity<List<UserRole>> getAllBoardsOfUser(@PathVariable Integer userId,
                                                             @AuthenticationPrincipal CustomUserDetails userDetails) {
        User user = userDetails.getUser();
